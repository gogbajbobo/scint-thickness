...
# %%
gos_mus = xraydb.material_mu("GOS", energies_eV)
gos_width_cm = 0.002
gos_absorption = 1 - np.exp(-gos_mus * gos_width_cm)

plt.plot(energies_keV, gos_absorption)
plt.grid()
plt.show()
...
gos_al_intensities = al_intensities * gos_absorption
...
for gos_al_intens in gos_al_intensities[::plot_step]:
    ax[2].semilogy(energies_keV, gos_al_intens)
ax[2].grid()
...
# %%
gos_sums = gos_al_intensities.sum(axis=-1)

# %%
res = np.linalg.lstsq(al_intensities, gos_sums, rcond=None)

# %%
plt.plot(res[0])
plt.plot(gos_absorption)
plt.grid()
plt.show()
